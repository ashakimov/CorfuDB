...
public class NettyLogUnitProtocol
    ...
    public CompletableFuture<WriteResult> setCommit(long address, boolean commit) {
        return handler.sendMessageAndGetCompletable(epoch, new NettyLogUnitCommitMsg(address, null, commit));
    }

    @Override
    public CompletableFuture<WriteResult> setCommit(Map<UUID, Long> streams, boolean commit) {
        return handler.sendMessageAndGetCompletable(epoch, new NettyLogUnitCommitMsg(-1L, streams, commit));
    }
    ...
    public CompletableFuture<ReadResult> read(long address) {
        return handler.sendMessageAndGetCompletable(epoch, new NettyLogUnitReadRequestMsg(address, null, -1L));
    }

    @Override
    public CompletableFuture<ReadResult> read(UUID stream, long localAddress) {
        return handler.sendMessageAndGetCompletable(epoch, new NettyLogUnitReadRequestMsg(-1L, stream, localAddress));
    }
